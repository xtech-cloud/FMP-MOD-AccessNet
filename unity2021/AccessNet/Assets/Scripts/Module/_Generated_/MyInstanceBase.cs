...
namespace XTC.FMP.MOD.AccessNet.LIB.Unity
{
    public class MyInstanceBase
    {
        ...
        public IPointViewBridge viewBridgePoint { get; set; }
        ...
        public void SetupBridges()
        {
            ...
            var facadePoint = entry_.getDynamicPointFacade(uid);
            var bridgePoint = new PointUiBridge();
            bridgePoint.logger = logger_;
            facadePoint.setUiBridge(bridgePoint);
            viewBridgePoint = facadePoint.getViewBridge() as IPointViewBridge;
        ...
        protected virtual void submitPointOnline(PointOnlineRequest _request)
        {
            var dto = new PointOnlineRequestDTO(_request);
            SynchronizationContext context = SynchronizationContext.Current;
            Task.Run(async () =>
            {
                try
                {
                    var reslut = await viewBridgePoint.OnOnlineSubmit(dto, context);
                    if (!LibMVCS.Error.IsOK(reslut))
                    {
                        logger_.Error(reslut.getMessage());
                    }
        ...
        protected virtual void submitPointOffline(PointOfflineRequest _request)
        {
            var dto = new PointOfflineRequestDTO(_request);
            SynchronizationContext context = SynchronizationContext.Current;
            Task.Run(async () =>
            {
                try
                {
                    var reslut = await viewBridgePoint.OnOfflineSubmit(dto, context);
                    if (!LibMVCS.Error.IsOK(reslut))
                    {
                        logger_.Error(reslut.getMessage());
                    }
        ...
        protected virtual void submitPointHeartBeat(PointHeartBeatRequest _request)
        {
            var dto = new PointHeartBeatRequestDTO(_request);
            SynchronizationContext context = SynchronizationContext.Current;
            Task.Run(async () =>
            {
                try
                {
                    var reslut = await viewBridgePoint.OnHeartBeatSubmit(dto, context);
                    if (!LibMVCS.Error.IsOK(reslut))
                    {
                        logger_.Error(reslut.getMessage());
                    }
        ...
        protected virtual void submitPointRetrieve(UuidRequest _request)
        {
            ...
            Task.Run(async () =>
            {
                try
                {
                    var reslut = await viewBridgePoint.OnRetrieveSubmit(dto, context);
                    if (!LibMVCS.Error.IsOK(reslut))
                    {
                        logger_.Error(reslut.getMessage());
                    }
        ...
        protected virtual void submitPointList(PointListRequest _request)
        {
            var dto = new PointListRequestDTO(_request);
            SynchronizationContext context = SynchronizationContext.Current;
            Task.Run(async () =>
            {
                try
                {
                    var reslut = await viewBridgePoint.OnListSubmit(dto, context);
                    if (!LibMVCS.Error.IsOK(reslut))
                    {
                        logger_.Error(reslut.getMessage());
                    }
